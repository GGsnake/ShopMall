...
public interface PayDao {
    ...
    @Insert("insert into jh_taobao_hot(pictUrl,shopTitle,title," +
            "commissionRate,coupon,zkFinalPrice,volume,numIid" +
            ",createTime,status) values " +
            "(#{pictUrl},#{shopTitle},#{title},#{commissionRate}" +
            ",#{coupon},#{zkFinalPrice},#{volume},#{numIid}," +
            ",now(),0)")
    Integer tsstt(Map<String,Object> map);
